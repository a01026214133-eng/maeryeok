// api/og.js
// 링크 공유 시 OG 태그 HTML 반환 — og:image는 /api/og-image가 서버에서 직접 생성
...
export default async function handler(req, res) {
  ...
  // OG 이미지: /api/og-image가 서버에서 직접 생성 (업로드 불필요)
  const imgParams = new URLSearchParams();
  if (year)  imgParams.set('year',  year);
  if (month) imgParams.set('month', month);
  if (day)   imgParams.set('day',   day);
  if (name)  imgParams.set('name',  name);
  const ogImageUrl = `${siteUrl}/api/og-image?${imgParams.toString()}`;
...
<body>
  <p>잠시만 기다려주세요... <a href="${esc(resultUrl)}">결과 보기</a></p>
</body>
</html>`;
